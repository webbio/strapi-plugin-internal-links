const getDomain = (uid) => {
	const pluginConfig = strapi.plugin('internal-link').service('config').getGlobalConfig();
...
const getSlug = (uid, entity) => {
	const globalConfig = strapi.service('plugin::internal-link.config').getGlobalConfig();
	const entityConfig = strapi.service('plugin::internal-link.config').getContentTypeConfig(uid);
